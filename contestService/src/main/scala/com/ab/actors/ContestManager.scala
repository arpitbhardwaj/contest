 ...
 *
 * Actor model
 *    Based on entities called actor
 *    Actors have state and behaviour
 *    Multiple actors form a actor system
 *    Actors from a system use message based asynchronous non blocking communication with in or external system
 *    Each actor comes with a message box where messages are stored and processed in FIFO Order
 *    Actors sends immutable messages
 *    Actors work in a hierarchy and the top actor is known as guardian actor
 *
 * Akka Actor is the implementation of Actor model
 *    provides classic actor and typed actor
 *    will focus on types actor in this project
 *    ActorRef are used to interact with actors
 */
      ...
      msg match {
        ...
        case StartContest(contestId) =>
          if(contests.contains(contestId) && !activeContests.contains(contestId)){
            ...
            //ask between actors with message adapter pattern
